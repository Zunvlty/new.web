...
function setupDeveloperPanel() {
    ...
    if (addProductForm) addProductForm.addEventListener('submit', async function(e) {
        ...
        let image = document.getElementById('newProductImage') ? document.getElementById('newProductImage').value : '';
        if (imageInput && imageInput.files && imageInput.files[0] && firebaseInitialized && storage) {
            try {
                ...
                image = imageInput.value || image;
            }
        }
        if (name && price && category && image) {
            if (firebaseInitialized && db) {
                try {
                    await db.collection('products').add({ name, price, category, image });
                } catch (e) {
                }
            }
